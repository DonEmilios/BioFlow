...
export const nodeRegistry: NodeRegistryEntry[] = [
  ...
  },
  {
    ...
    category: "process",
    description: "Differential abundance from a metabolomics CSV: log2 fold change, Welch's t-test, and BH-adjusted p-values per metabolite. Answers 'which molecules change'.",
    icon: "Activity",
    input_types: ["csv"],
    output_types: ["json"],
    params: [
  ...
  },
  {
    ...
    category: "process",
    description: "Maps changed metabolites onto metabolic pathways and tests which are over-represented (hypergeometric). Answers 'why do they change'.",
    icon: "Network",
    input_types: ["json"],
    output_types: ["json"],
    params: [
      { id: "padj_cutoff", label: "Significance cutoff", type: "number", default: 0.05, required: true, help_text: "Metabolites below this adjusted p-value form the 'changed' set fed into enrichment." },
  ...
  },
  {
    ...
    category: "process",
    description: "Preprocess an omics matrix: log2 transform and/or scaling (autoscale / Pareto). Feeds PCA and clustering.",
  ...
  },
  {
    ...
    label: "PCA",
    category: "viz",
    description: "Principal component analysis via real eigendecomposition. Reveals sample structure, group separation, and batch effects.",
    icon: "ScatterChart",
    input_types: ["csv", "json"],
  ...
  },
  {
    ...
    category: "process",
    description: "Unsupervised k-means clustering of samples with a silhouette score, plus cluster/group agreement when labels are known.",
    icon: "Boxes",
    input_types: ["csv", "json"],
    output_types: ["json"],
    params: [
